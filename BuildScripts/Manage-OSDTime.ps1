...
.DESCRIPTION 
    The script works in 2 parts:
        ...
        Part2:
            ...
            3b)If the -tattoo switch is omitted,it will creates a TS variable called SVHA_OSDInstallTime which can be easily read by the New-OSDTattoo.ps1
...
#>
    ...
    Param(
        ...
        [string]$RegistryPath = "HKLM:SOFTWARE\SVHA",
            ...
            #getting existing startTime
                ...
                if (!($StartTime)){
                    ...
                    #Getting end time
                        ...
                        $SVHA_endTime = ([System.DateTimeOffset]::UtcNow).UtcDateTime
                        $EndTimeTicks = ([System.DateTimeOffset]::UtcNow).UtcTicks
                        $TimeSpan = new-object timespan(($EndTimeTicks - $StartTimeUTCTicks))
                        $SVHA_OSDInstallTime = [math]::Round($TimeSpan.TotalMinutes,2)

                        if (!($SVHA_OSDInstallTime)){
                            $SVHA_OSDInstallTime = 'Undefined'
                            ...
                            if ($tattoo){
                                $FullRegPath = join-path -Path "HKLM:\SOFTWARE\SVHA" -ChildPath $Root
                                New-RegistryItem -RegistryPath $FullRegPath -RegistryString "SVHA_OSDInstallationTime" -RegistryValue $SVHA_OSDInstallTime
                                log-message "[OSDTIME]Tattooed execution time value $($SVHA_OSDInstallTime) to registry (only). $($FullRegPath)"
                            }
                            else{
                                
                                $tsenv.Value("SVHA_OSDInstallTime") = $SVHA_OSDInstallTime 
                                ...
                                log-Message "[OSDTIME]Installlation time --> $($SVHA_OSDInstallTime)"
                            }
